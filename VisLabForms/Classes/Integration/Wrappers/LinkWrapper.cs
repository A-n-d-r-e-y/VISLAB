...
namespace VisLab.Classes
{
    public class LinkWrapper
    {
        ...
        /// Reference line through the center of the link (array of VARIANTs)
        /// </summary>
        private IEnumerable<PointF> GetPointsF()
        {
            var enumerator = ((object[])link.get_AttValue("POINTS")).GetEnumerator();

            while (enumerator.MoveNext())
            {
                yield return new PointF((float)(enumerator.Current as WorldPoint).X / 2 + 100, 605 - (float)(enumerator.Current as WorldPoint).Y / 2);
            }
        }
    }
}
